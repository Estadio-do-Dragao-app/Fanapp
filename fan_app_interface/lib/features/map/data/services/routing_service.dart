...
class RoutingService {
  static const String baseUrl = 'http://10.0.2.2:8002'; // Alterar para produção
  ...
  Future<RouteModel> getEvacuationRoute({required String fromNode}) async {
    final uri = Uri.parse('$baseUrl/api/route/evacuation').replace(
      queryParameters: {'from_node': fromNode},
    );
  ...
  Future<RouteModel> getMultiDestinationRoute({
    ...
    if (response.statusCode == 200) {
      ...
      throw Exception('Failed to get multi-destination route: ${response.statusCode}');
    }
  }
