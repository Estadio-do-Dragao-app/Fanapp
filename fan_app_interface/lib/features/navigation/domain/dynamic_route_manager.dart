...
class DynamicRouteManager {
  ...
  Future<void> _checkAndRecalculateIfNeeded(double userX, double userY) async {
    ...
    for (int i = 0; i < _currentRoute!.waypoints.length - 1; i++) {
      final wp1 = _currentRoute!.waypoints[i];
      final wp2 = _currentRoute!.waypoints[i + 1];
    ...
    print('[DynamicRouteManager] Distância à rota: ${minDistanceToRoute.toStringAsFixed(1)}m');
  ...
  Future<void> _recalculateRoute(double userX, double userY) async {
    ...
    try {
      // Encontrar nó mais próximo da posição atual
      final nearestNode = _findNearestNode(userX, userY);
      
      // Calcular nova rota
      final newRoute = await _routingService.getRoute(
        fromNode: nearestNode.id,
        toNode: destinationNodeId,
      );
      
      _currentRoute = newRoute;
      
      print('[DynamicRouteManager] ✅ Nova rota calculada: ${newRoute.waypoints.length} waypoints');
  ...
  double _pointToLineDistance(
    ...
    final t = max(0.0, min(1.0, ((px - x1) * (x2 - x1) + (py - y1) * (y2 - y1)) / pow(lineLength, 2)));
